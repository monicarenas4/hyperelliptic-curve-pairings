...
def generate_jacobian_cp8(u, X, Y, lx, ly, l, k = 8, a = 3):
    ...
    Fpw = Fp['w']; (w,) = Fpw._first_ngens(1) # Fpw: polynomial ring in w with coefficients in Fp
    b = 1
    while not (w**8 + b).is_irreducible():
        b = b+1
    print("Fp8 = Fp[w]/(w^8+{})".format(b)) # Fp8 = Fp[w]/(w^8 + b)
    # Fp8: degree 8 extension of Fp.
    Fp8 = Fp.extension(w**8 + b, names=('w',)); (w,) = Fp8._first_ngens(1)

    # Constructing the curve and Jacobian over the extension field Fp**8
    Fp8x = Fp8['x']; (x,) = Fp8x._first_ngens(1) # Fp8x: ring of polynomials in x, with coefficients in Fp8
